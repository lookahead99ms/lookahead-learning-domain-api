...
/** Platform fixtures contain subject IDs and grants only, never authentication credentials. */
@Component
public class PlatformFixtureGuard {
    public PlatformFixtureGuard(Environment environment) {
        ...
        if ((fixtureProfile || seed || author) && (!local || production || !fixtureProfile))
            throw new IllegalStateException("Platform fixtures require local-test and local deployment only");
